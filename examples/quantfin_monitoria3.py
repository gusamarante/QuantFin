...
start_du = (2 * avg_du + holding_period) / 2  # This assures that, on average, the position maturity is avg_du

# first day - build initial position
aux = df_raw[df_raw['reference_date'] == time_menu[0]].sort_values('du')
...
df_backtest.loc[time_menu[0], 'contract 1'] = current_contracts.iloc[0]['contract']
df_backtest.loc[time_menu[0], 'contract 2'] = current_contracts.iloc[1]['contract']
df_backtest.loc[time_menu[0], 'quant 1'] = notional * shares[0, 0] / current_contracts.iloc[0]['theoretical_price']
df_backtest.loc[time_menu[0], 'quant 2'] = notional * shares[1, 0] / current_contracts.iloc[1]['theoretical_price']
df_backtest.loc[time_menu[0], 'notional'] = notional

next_rebalance = time_menu[0] + pd.offsets.BDay(holding_period)

for date, datem1 in tqdm(zip(time_menu[1:], time_menu[:-1])):
    if date == pd.to_datetime('2019-07-31'):
        a = 1
