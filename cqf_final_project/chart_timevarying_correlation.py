...
ax.set_xlim((df_corr.index[0], df_corr.index[-1]))
locators = mdates.YearLocator()
ax.xaxis.set_major_locator(locators)
ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
plt.tight_layout()
plt.savefig(file_path + f'figures/Correlation change over time {ccy1}{ccy2}.pdf', pad_inches=0)
plt.show()

# Histogram
histograms = df_corr.hist(figsize=(10, 4), bins=20)
for array in histograms:
    for subplot in array:
        subplot.set_xlim((-1,1))

plt.tight_layout()
plt.savefig(file_path + f'figures/Correlation histogram {ccy1}{ccy2}.pdf', pad_inches=0)
plt.show()
