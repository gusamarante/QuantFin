...
import pandas as pd
import numpy as np
import blpapi  # Not supported on mac
...
class BBG(object):
    ...
    def fetch_series(securities, fields, startdate, enddate, period="DAILY", calendar="ACTUAL", fx=None,
                     fperiod=None, verbose=False, nonTradingDayFillOption=None):
        """
        ...
        :param nonTradingDayFillOption: 'NON_TRADING_WEEKDAYS', 'ALL_CALENDAR_DAYS' or 'ACTIVE_DAYS_ONLY'
        :return:  DataFrame or Multi-index DataFrame (if more than one field is passed)
        """

        # TODO allow for 'securities' to be a dict, with tickers on the keys and names on the values.
        ...
        try:
            ...
            if not (nonTradingDayFillOption is None):
